...
`models/multilabel/label2id.json` is missing. Registries hold 43 PR + 45
EVA entries keyed to the multilabel labels; ~78 of the 88 entries are
generated by `template_handler(intent_label, channel, field_path)`,
the remaining 7 PR specials (`get_lidar` + 6 `set_*` verbal acks) are
hand-rolled in `src/core/responder/handlers_pr.py`.
