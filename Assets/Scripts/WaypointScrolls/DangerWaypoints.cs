...
{
    public class DangerWaypoints : MonoBehaviour
    {
        ...
        private void Start()
        {
            Debug.Log("DangerWaypoints Start method called.");
            ...
            list.OnVisible = (go, i) =>
            {
                ...
                if (i < 0 || i >= navigationController.DangerWaypointList.Count)
                {
                    Debug.LogWarning($"Index {i} is out of range for danger waypoint list.");
                    return;
                }

                Waypoint waypoint = navigationController.DangerWaypointList[i];
