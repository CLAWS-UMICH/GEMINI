...
{
    public class GEOWaypoints : MonoBehaviour
    {
        ...
        private void Start()
        {
            Debug.Log("GEO Start method called.");
            ...
            list.OnVisible = (go, i) =>
            {
                ...
                if (i < 0 || i >= navigationController.GeoWaypointList.Count)
                {
                    Debug.LogWarning($"Index {i} is out of range for danger waypoint list.");
                    return;
                }

                Waypoint waypoint = navigationController.GeoWaypointList[i];
