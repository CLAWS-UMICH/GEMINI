...
{
    public class POIWaypoints : MonoBehaviour
    {
        ...
        private void Start()
        {
            Debug.Log("POI Start method called.");
            ...
            list.OnVisible = (go, i) =>
            {
                ...
                if (i < 0 || i >= navigationController.POIWaypointList.Count)
                {
                    Debug.LogWarning($"Index {i} is out of range for danger waypoint list.");
                    return;
                }

                Waypoint waypoint = navigationController.POIWaypointList[i];
